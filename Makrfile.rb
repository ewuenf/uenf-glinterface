...
def configureBase()
  ...
  compilerConfig["compiler.cFlags"] = " -Wall -Werror -fexceptions -std=c++11"
...
def buildIt()
  ...
  $build.saveAfterBlock do
    ...
    #myLibTask = Makr.makeStaticLib($buildDir + "/libuenf-graph.a", $build, tasks, $build.getConfig("CompileTaskCPP"))
    myProg = Makr.makeProgram($buildDir + "/uenftest", $build, tasks, $build.getConfig("CompileTaskCPP"))
